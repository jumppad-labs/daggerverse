...
func (r Notorize) MarshalJSON() ([]byte, error) {
	var concrete struct {
		P12Cert         *File
	...
	concrete.P12Cert = r.P12Cert
	concrete.P12CertPassword = r.P12CertPassword
	concrete.NotoryKey = r.NotoryKey
	concrete.NotoryKeyID = r.NotoryKeyID
	concrete.NotoryIssuer = r.NotoryIssuer
...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (_ any, err error) {
	...
	case "Notorize":
		...
		case "TestNotorize":
			...
			var keyIssuer string
			if inputArgs["keyIssuer"] != nil {
				err = json.Unmarshal([]byte(inputArgs["keyIssuer"]), &keyIssuer)
				if err != nil {
					panic(fmt.Errorf("%s: %w", "failed to unmarshal input arg keyIssuer", err))
				}
			}
			return (*Notorize).TestNotorize(&parent, ctx, file, cert, password, key, keyId, keyIssuer)
		default:
				...
				dag.TypeDef().WithObject("Notorize").
					...
					WithFunction(
						dag.Function("TestNotorize",
							...
							WithArg("key", dag.TypeDef().WithObject("File")).
							WithArg("keyId", dag.TypeDef().WithKind(StringKind)).
							WithArg("keyIssuer", dag.TypeDef().WithKind(StringKind))).
