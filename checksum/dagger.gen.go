...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (_ any, err error) {
	...
	case "Checksum":
		...
		case "CalculateFromUrl":
			...
			return (*Checksum).CalculateFromUrl(&parent, ctx, url)
		case "CalculateFromFile":
			var parent Checksum
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
				panic(fmt.Errorf("%s: %w", "failed to unmarshal parent object", err))
			...
			return (*Checksum).CalculateFromFile(&parent, ctx, file)
		default:
	...
	case "":
		return dag.Module().
			WithObject(
				dag.TypeDef().WithObject("Checksum").
					WithFunction(
						dag.Function("CalculateFromUrl",
							dag.TypeDef().WithKind(StringKind)).
							WithDescription("CalculateChecksum will calculate the checksum of a given URL").
							WithArg("url", dag.TypeDef().WithKind(StringKind))).
					WithFunction(
						dag.Function("CalculateFromFile",
							dag.TypeDef().WithKind(StringKind)).
							WithArg("file", dag.TypeDef().WithObject("File")))), nil
	default:
