...
func (r Vault) MarshalJSON() ([]byte, error) {
	var concrete struct {
		Namespace string
		Host      string
...
func (r UserpassAuth) MarshalJSON() ([]byte, error) {
	var concrete struct {
		Username string
		Password string
		Path     string
...
func (r JWTAuth) MarshalJSON() ([]byte, error) {
	var concrete struct {
		Token string
		Role  string
		Path  string
...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (_ any, err error) {
	...
	case "Vault":
		...
		case "WithNamespace":
			...
			return (*Vault).WithNamespace(&parent, namespace), nil
		case "WithHost":
			var parent Vault
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return (*Vault).WithHost(&parent, host), nil
		case "WithUserpassAuth":
			var parent Vault
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
				panic(fmt.Errorf("%s: %w", "failed to unmarshal parent object", err))
		...
		case "TestRead":
			...
			return (*Vault).TestRead(&parent, ctx, host, namespace, username, password, secret)
		case "TestKVGet":
			var parent Vault
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
		...
		return dag.Module().
			WithObject(
				dag.TypeDef().WithObject("Vault").
					WithFunction(
						dag.Function("WithNamespace",
							dag.TypeDef().WithObject("Vault")).
							WithDescription("WithNamespace sets the namespace for the Vault client").
							WithArg("namespace", dag.TypeDef().WithKind(StringKind))).
					WithFunction(
						dag.Function("WithHost",
							dag.TypeDef().WithObject("Vault")).
							WithDescription("WithHost sets the host for the Vault client").
							WithArg("host", dag.TypeDef().WithKind(StringKind))).
					WithFunction(
						dag.Function("WithUserpassAuth",
							dag.TypeDef().WithObject("Vault")).
