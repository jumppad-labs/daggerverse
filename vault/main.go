...
func (v *Vault) KVGet(
	...
	if err != nil {
		return nil, fmt.Errorf("unable to get secret details: %w", err)
	}
	...
	case "1":
		...
		respSecret := dag.SetSecret("vault_response", string(js))
	...
	return nil, fmt.Errorf("unsupported version: %s", dets.Version)
}
...
func (v *Vault) TestWrite(
	...
	s, err := v.Write(ctx, secret, params)
	if err != nil {
		return "", err
	}
